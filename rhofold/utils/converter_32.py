...
class RNAConverter():
    ...
    def build_cords(self, msa_tokens, fram, angl, rtn_cmsk=False):
        max_len=rhofold_config.max_len_seq
        # initialization
        device = angl.device

        angl = angl / (torch.norm(angl, dim=3, keepdim=True) + self.eps)
        rigid = Rigid.from_tensor_7(fram, normalize_quats=True)# frame -> rigid
        fram = rigid.to_tensor_4x4()    # rigid -> frame
        rot = fram[:,:,:3,:3]
        ...
        cords=[]
        cmsks=[]
        for i in range(msa_tokens.shape[0]): #batch
            for k in range(max_len):
                if msa_tokens[i,0,k].tolist() == 8: # when '-' appears, RNA ends
                    n_resds= k
                else:
                    n_resds=max_len

            fmsk = torch.ones((n_resds, 1), dtype=torch.int8, device=device)
            fmsk = torch.cat((fmsk, torch.zeros((max_len-n_resds, 1), dtype=torch.int8, device=device)), dim = 0)
            amsk = torch.ones((n_resds, RNA_CONSTANTS.N_ANGLS_PER_RESD_MAX), dtype=torch.int8, device=device)
            amsk = torch.cat((amsk, torch.zeros((max_len-n_resds, RNA_CONSTANTS.N_ANGLS_PER_RESD_MAX), dtype=torch.int8, device=device)), dim = 0)
            cord = torch.zeros((n_resds, RNA_CONSTANTS.ATOM_NUM_MAX, 3), dtype=torch.float32, device=device)
            cord = torch.cat((cord, torch.zeros((max_len-n_resds, RNA_CONSTANTS.ATOM_NUM_MAX, 3), dtype=torch.float32, device=device)), dim = 0)
            cmsk = torch.zeros((n_resds, RNA_CONSTANTS.ATOM_NUM_MAX), dtype=torch.int8, device=device)
            cmsk = torch.cat((cmsk, torch.zeros((max_len-n_resds, RNA_CONSTANTS.ATOM_NUM_MAX), dtype=torch.int8, device=device)), dim = 0)

            for resd_name in RNA_CONSTANTS.RESD_NAMES: # AGUC
                idxs = [x for x in range(n_resds) if token2toRNA[msa_tokens[i,0,x].tolist()] == resd_name]
                if len(idxs) == 0:
                    continue
                #print('fram[idxs,i]{}'.format(fram[idxs,i].unsqueeze(1).shape)) #torch.Size([11, 1, 4, 3])

                cord[idxs], cmsk[idxs] = self.__build_cord(resd_name, fram[idxs], fmsk[idxs], angl[i,idxs], amsk[idxs])

            if i == 0:
                cords=cord.unsqueeze(0)
                cmsks=cmsk.unsqueeze(0)
            else :                
                cords = torch.cat((cords, cord.unsqueeze(0)), dim=0)
                cmsks = torch.cat((cmsks, cmsk.unsqueeze(0)), dim=0)

        #print(f'cords.shape {cords.shape}') #torch.Size([2, 100, 23, 3])
        #print(f'cmsks.shape {cmsks.shape}') #torch.Size([2, 100, 23])

        return (cords, cmsks) if rtn_cmsk else (cord)
    ...
    def build_cords(self, msa_tokens, fram, angl, rtn_cmsk=False): ###
        ...
        device = angl.device
        max_len = msa_tokens.shape[2]
        print('max_len {}'.format(max_len))

        batch_dim = fram.shape[0]
        print(f'batch_dim {batch_dim}') # seq length

        print('angl {}'.format(angl))
        print('angl {}'.format(angl.shape)) #angl torch.Size([3, 100, 6, 2])

        #angl = angl.squeeze(dim=0) / (torch.norm(angl.squeeze(dim=0), dim=2, keepdim=True) + self.eps) ###   dim=2 batch_size =2 のとき機能しない。
        angl = angl / (torch.norm(angl, dim=3, keepdim=True) + self.eps)
        print('angl {}'.format(angl))
        print('angl {}'.format(angl.shape)) #angl torch.Size([3, 100, 6, 2])
        ...
        fram = torch.cat([rot, tsl], dim=2)[:,:,:4,:3].permute(1,0,2,3) #何をしているのか

        print(f'fram {fram.shape}')#torch.Size([28, 1, 4, 3]) torch.Size([100, 2, 4, 3])
        
        
        cords=[]
        cmsks=[]
        for i in range(msa_tokens.shape[0]):
            for k in range(max_len):
                if msa_tokens[i,0,k].tolist() == 8:
                    n_resds= k
            ...
            for j in range(max_len): # max_len
                if msa_tokens[i,0,j].tolist() == 8:
                    pass 
                else :
                    resd_name = token2toRNA[msa_tokens[i,0,j].tolist()]   
                    xx = fram[j,i].unsqueeze(0).unsqueeze(0)
                    print(xx.shape) #[1,1,4,3]
                    print(xx[0,0,:3])
                    print(xx[0,0,3])

                    #print(f'fmsk[j] {fmsk[j].unsqueeze(1)}')
                    #print(f'angl[i,j] {angl[j].unsqueeze(1)}')
                    #print(f'amsk[j] {amsk[j].unsqueeze(1)}')
                    
                    cord[j], cmsk[j] =\
                    self.__build_cord(resd_name, fram[j,i].unsqueeze(0).unsqueeze(0), fmsk[j].unsqueeze(0), angl[i,j].unsqueeze(0), amsk[j].unsqueeze(0))  ###
                #print(cord[j])
                #print(cord[j].shape)
            #print(cord)
            #print(cord.shape) #torch.Size([100, 23, 3])
    ...
    def __build_cord(self, resd_name, fram, fmsk, angl, amsk):
        ...
        # initialization
        device = fram.device
        n_resds = fram.shape[0] # seq length
        ...
        trans_dict = {'main': (fram[:, 0, :3], fram[:, 0, 3])} #frame
