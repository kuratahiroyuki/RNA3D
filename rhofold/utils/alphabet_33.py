...
#rnaseq_toks = {'toks': ['A', 'C', 'G', 'U', 'R', 'Y', 'K', 'M', 'S', 'W', 'B', 'D', 'H', 'V', 'N', '-'] } # why they are different?
...
def get_msa_feature(pdb_id, seq,
                    msa_depth,
                    batch_converter = RNAAlphabet.from_architecture('RNA').get_batch_converter()):

    #msa_data = [read_msa(msa_path, msa_depth)]
    msa_data = [[(pdb_id, seq)]] ###[[('3meiA', 'CCGCCGCGCCAUGCCUGUGGCGG')]]
    #print('msa_data{}'.format(msa_data))

    _, _, msa_batch_tokens = batch_converter(msa_data)
    #print('msa_batch_tokens :{}'.format(msa_batch_tokens))
    #print('msa_batch_tokens.shape :{}'.format(msa_batch_tokens.shape))
    # remove [cls] token in msa_batch_tokens
    fea1d = msa_batch_tokens.squeeze(0).data.cpu().numpy().transpose((1, 0))[1:, :]
    #print('fea1d :{}'.format(fea1d))
    #print('fea1d.shape :{}'.format(fea1d.shape))
    #print('fea1d[:, :].transpose((1, 0)) :{}'.format(torch.LongTensor(fea1d[:, :].transpose((1, 0)))))
  
    return torch.LongTensor(fea1d[:, :].transpose((1, 0)))

def get_features(pdb_id, seq, msa_fpath, msa_depth = 128):
    '''
    Get features from MSA
    '''

    #msa_tokens = get_msa_feature(msa_path=msa_fpath, msa_depth=msa_depth) 
    msa_tokens = get_msa_feature(pdb_id, seq, msa_depth=msa_depth) ###

    rna_fm_tokens = get_rna_fm_token(pdb_id, seq)

    #print('msa_tokens.shape :{}'.format(msa_tokens.shape))
    #print('rna_fm_tokens :{}'.format(rna_fm_tokens))
    #print('rna_fm_tokens.shape :{}'.format(rna_fm_tokens.shape))
