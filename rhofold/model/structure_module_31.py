...
from rhofold.utils.converter_32 import RNAConverter
...
class RefineNet(nn.Module):
    ...
    def forward(self, tokens, cords):
        ...
        tokens = tokens[:, 0, :]
        #print('tokens.shape {}'.format(tokens.shape)) #[B,43]
        tokens = tokens.unsqueeze(-1).repeat(1, 1, RNA_CONSTANTS.ATOM_NUM_MAX)
        b, l, n = tokens.shape
        #print('tokens.shape {}'.format(tokens.shape)) # [B, 43, 23]
        #print('cords.shape {}'.format(cords.shape)) # [B, 23, 3]

        cords = cords.reshape([b, l, n, 3]) #[B, 32, 23, 3]
...
class StructureModule(nn.Module):
    ...
    def forward(
        ...
        #print('In structure, mask {}'.format(mask))
        #print('In structure, s {}'.format(s))
        ...
        for i in range(n_blocks_act):
            ...
            # [*, N]
            #print('s {}'.format(s.shape)) # torch.Size([B, 10, 384])
            rigids = rigids.compose_q_update_vec(self.bb_update(s))  #  [*, N_res, C_s] =>   [*, N_res, 6]  = > Rotation
            #print('rigids {}'.format(rigids.shape)) #torch.Size([B, 10])

            # [*, N, 7, 2]
            unnormalized_angles, angles = self.angle_resnet(s, s_initial)

            scaled_rigids = rigids.scale_translation(self.trans_scale_factor)
            #print('scaled_rigids {}'.format(scaled_rigids.shape)) #torch.Size([B, 10])
            #print('scaled_rigids.to_tensor_7() {}'.format(scaled_rigids.to_tensor_7().shape)) #torch.Size([B, 10, 7])
            
            preds = {
                "frames": scaled_rigids.to_tensor_7(),  #quats+trans
        ...
        #print('outputs[frames] {}'.format(outputs[0]['frames'].shape)) #　torch.Size([B, 10, 7])
        outputs = dict_multimap(torch.stack, outputs) #何をするのか　再帰関数

        #print('outputs[frames] {}'.format(outputs['frames'].shape)) #　torch.Size([B, 2, 100, 7])
        #print('outputs[frames][-1] {}'.format(outputs['frames'][-1].shape)) #　torch.Size([2, 100, 7]) 
             
        cords, mask = self.converter.build_cords(msa_tokens, outputs['frames'][-1], outputs['angles'][-1], rtn_cmsk=True) ### generation of 3d coords
        #print('cords {}'.format(cords.shape)) #torch.Size([100, 23, 3]) #is it funny?  Batchが消えた。
        cord_list = [[cords, mask]]
        

        outputs['cord_tns_pred'] = [ self.refinenet(msa_tokens, cord[0].reshape([s.shape[0], -1, 3])) for cord in cord_list]  ### 
        outputs["cords_c1'"] = [cord[0][:, :, 1, :] for cord in cord_list] ### tensor change

        #print('outputs cord_tns_pred {}'.format(outputs['cord_tns_pred'][0].shape)) #torch.Size([2, 2300, 3])
        #print('outputs cords_c1 {}'.format(outputs["cords_c1'"][0].shape)) #([2, 100, 3])
