...
from rhofold.utils.converter_31 import RNAConverter
...
class RefineNet(nn.Module):
    ...
    def forward(self, tokens, cords):
        ...
        tokens = tokens[:, 0, :]
        print('tokens.shape {}'.format(tokens.shape)) #[1,43]
        tokens = tokens.unsqueeze(-1).repeat(1, 1, 23)
        b, l, n = tokens.shape
        print('tokens.shape {}'.format(tokens.shape)) # [1, 43, 23]
        print('cords.shape {}'.format(cords.shape)) # [1, 23, 3]

        cords = cords.reshape([b, l, n, 3]) #[1, 32, 23, 3]
...
class StructureModule(nn.Module):
    ...
    def forward(
        ...
        for i in range(n_blocks_act):
            ...
            # [*, N]
            #print('s {}'.format(s.shape)) # torch.Size([1, 10, 384])
            rigids = rigids.compose_q_update_vec(self.bb_update(s))  #  [*, N_res, C_s] =>   [*, N_res, 6]  = > Rotation
            #print('rigids {}'.format(rigids.shape)) #torch.Size([1, 10])

            # [*, N, 7, 2]
            unnormalized_angles, angles = self.angle_resnet(s, s_initial)

            scaled_rigids = rigids.scale_translation(self.trans_scale_factor)
            #print('scaled_rigids {}'.format(scaled_rigids.shape)) #torch.Size([1, 10])
            #print('scaled_rigids.to_tensor_7() {}'.format(scaled_rigids.to_tensor_7().shape)) #torch.Size([1, 10, 7])
        ...
        #print('outputs[frames] {}'.format(outputs[0]['frames'].shape)) #　torch.Size([1, 10, 7])
        outputs = dict_multimap(torch.stack, outputs) #何をするのか　再帰関数

        #print('outputs[frames] {}'.format(outputs['frames'].shape)) #　torch.Size([8, 1, 10, 7])
        #print('outputs[frames][-1] {}'.format(outputs['frames'][-1].shape)) #　torch.Size([1, 10, 7])


        print('seq {}'.format(seq))
        
        cords, mask = self.converter.build_cords(seq, outputs['frames'][-1], outputs['angles'][-1], rtn_cmsk=True) # generation of 3d coords
        cord_list = [[cords, mask]]

        print('cords {}'.format(cords.shape)) #torch.Size([1, 23, 3]) #funny
