...
class MSANet(nn.Module):
    def __init__(self,
        ...
        self.embed_tokens = nn.Embedding(d_msa, d_model, padding_idx = 8)
        if self.is_pos_emb:
            self.embed_positions = LearnedPositionalEmbedding(max_len, d_model, 8)
    ...
    def forward(self, tokens):
        ...
        #print('msa_fea:{}'.format(msa_fea)) #zero padding works.

        #print('self.embed_positions(tokens.reshape(B * K, L)):{}'.format(self.embed_positions(tokens.reshape(B * K, L))))
        #print('msa_fea.shape:{}'.format(msa_fea.shape))
        #print('self.embed_positions(tokens.reshape(B * K, L)).view(msa_fea.size()).shape: {}'.format(self.embed_positions(tokens.reshape(B * K, L)).view(msa_fea.size()).shape)) #torch.Size([1, 2, 23, 256])

        if self.is_pos_emb:
            msa_fea += self.embed_positions(tokens.reshape(B * K, L)).view(msa_fea.size())
        #print('msa_fea in msa.py:{}'.format(msa_fea)) #zero padding works.
