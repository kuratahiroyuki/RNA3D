...
class DistHead(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('logits_dist0 {}'.format(logits_dist0[0,1,1,:])) #logits_dist0 torch.Size([4, 40, 20, 20]) B bin L L
...
class pLDDTHead(nn.Module):
    ...
    def forward(self, sfea_tns):
        ...
        plddt_local = torch.sum(self.bin_vals * self.sfmx(logits), dim=2) #[2,20]
        #print('self.bin_vals * self.sfmx(logits) {}'.format((self.bin_vals * self.sfmx(logits)).shape)) #  torch.Size([1, 10, 50]) #no_bins
...
class TMScoreHead(nn.Module):
    ...
    def forward(self, z):
        ...
        """
        # [*, N, N, no_bins]
        print('tm z {}'.format(z.shape)) #Torch.Size([3, 100, 100, 128])
