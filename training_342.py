...
def main(config):
    ...
    with timing('RhoFold Training', logger=logger):
        config.device = get_device(config.device)
        logger.info(f'    Training using device {config.device}')

        # input RNA sequence and groud truth 3d coordinates
        outfile = './example/output/temp' 
        max_len = rhofold_config.max_len_seq  #20 30 100 1000    
        max_epoch = 1 # 1 5 50 500 ...
        # Choose the language model or nn.Embedding
        rhofold_config.model.msa_embedder.rna_fm['enable'] = False # RNA-FM language model is used of not. RNA-FM should be downloaded.
        #print(rhofold_config.model.msa_embedder.rna_fm['enable'])
        rhofold_config.data_module.data_loaders.batch_size = 8
        infile = './example/rho2_coords_%s.pkl'%max_len
        ...
        batch = pdb_data_input(rho_coords, config, max_len) # from RhoFold31/inference_36.py
        #random.shuffle(batch)
        #print([' {}'.format(batch[i]['pdb']) for i in range(6)])
        #print('tokens: {} '.format(batch[0]['tokens'].shape)) #[1,20]
        #print('rna_fm_tokens: {} '.format(batch[0]['rna_fm_tokens'].shape)) #[20]

        batch_size = rhofold_config.data_module.data_loaders.batch_size

        #training_set = PDB_Manager(batch[0:1*batch_size]) ###
        training_set = PDB_Manager(batch)
        training_generator = DataLoader(training_set, **rhofold_config.data_module.data_loaders)

        #valid_set = PDB_Manager(batch[batch_size:2*batch_size]) 
        #valid_generator = DataLoader(valid_set, **rhofold_config.data_module.data_loaders)
     
        model = RhoFold(rhofold_config).to(config.device)
        opt = optim.Adam(params = model.parameters(), lr = 0.001*0.01)
        ...
        for epoch in range(max_epoch):
            ...
            for batch_i, data in enumerate(training_generator): #stack expects each tensor to be equal size 
                                                                #RuntimeError: stack expects each tensor to be equal size, but got [20, 7] at entry 0 and [27, 7] at entry 3

                print('idx: {}, data[frames]: {}'.format(batch_i, data['frames'].shape)) #torch.Size([B, 20, 7])
                #print('idx, tokens {} {} '.format(batch_i, data['tokens'].shape)) #[B,1,20]
                #print('frames_mask {}'.format(data['frames_mask'].shape)) #[B,20]
                #print(data['tokens'].squeeze(1)*data['frames_mask'])
                
                torch.autograd.set_detect_anomaly(True)
                opt.zero_grad()

                output = model(tokens=data['tokens'].to(config.device),        #MSA
                               rna_fm_tokens=data['rna_fm_tokens'].to(config.device),  #RNA-FM
                ...
                print('cum_loss: {}'.format(cum_loss)) #tensor(14.9449, device='cuda:0', dtype=torch.float64)
                ...
                print('plddt in loss: {}'.format(measures['plddt'])) #duplication output['plddt'] in rhofold.py
                print('plddt {}'.format(output['plddt'][1].detach().cpu() )) #torch.Size([B]) tensor([0.4944, 0.4967], device='cuda:0', grad_fn=<MeanBackward1>)
            ...
            training_losses.append([ float(losses["loss"].cpu()), float(losses["fape"].cpu()), float(losses["rmsd"].cpu()), float(losses["plddt_loss"].cpu()), float(losses["tm_loss"].cpu()),  float(measures["lddt"].cpu()), float(output['ptm']) ])
        ...
        output_dict['loss_curve'] = training_losses
        ...
        for idx in range(output['all_atom_pred_pos'].shape[0]):
            ...
            #print(output['c4_'][0].shape) # torch.Size([B, 40, 100, 100])

            # Secondary structure, .ct format
            ss_prob_map = torch.sigmoid(output['ss'][idx, 0]).data.cpu().numpy()  #torch.Size([3, 1, 100, 100])
